...
/**
 * 
 * @param {number} position Posição em milissegundos do tempo do podcast
 * @returns {string} Informação do tempo no formato h:mm:ss ou mm:ss
 */
...
/**
 * Configura o funcionamento de todos os podcasts em tela
 */
const addPodcastsIframes = () => {
    // Seletor dos objetos com role podcast
    const allPodcasts = document.querySelectorAll('[role="podcast"]');
    
    // Aplica as alterações em todos os podcasts no documento
    allPodcasts.forEach(podcast => {
        // O objeto podcast precisa ter um atributo data-url para funcionar corretamente
        const url = podcast.dataset.url;        
        if(url){
            // Adiciona um iframe invisível como elemento filho
            ...
            // Instancia o objeto de controle de mídia
            const widget = SC.Widget(iframe);
            
            // Elementos de progresso e controle de tempo
            const progressElement = podcast.querySelector(".progress-bar .progress");
            ...
            if(progressElement && currentTimeElement){
                // aguarda o elemento se carregar por completo antes de coletar informações do áudio
                widget.bind(SC.Widget.Events.READY, () => {
                    widget.getDuration(duration => {
                        progressElement.dataset.duration = duration;
                        totalDurationElement.innerText = convertPositionToTime(duration);
                ...
                // ações de monitoramento enquanto o podcast está sendo tocado
                widget.bind(SC.Widget.Events.PLAY_PROGRESS, () => {
                    widget.getPosition(position => {
                        progressElement.dataset.position = position;
                        currentTimeElement.innerText = convertPositionToTime(position);
                    });
                })
            }else{
                console.error("Classes .progress-bar e/ou .progress não especificadas no elemento.")
            }
            ...
            if(playButton){
                ...
                console.error("Classe .play-button não especificada no elemento.")
            }
            ...
            if(rewindButton){
                ...
                console.error("Classe .rewind-button não especificada no elemento.")
            }
            ...
            if(forwardButton){
                forwardButton.addEventListener("click", (e) => {
                    widget.getPosition(position => {
                        widget.seekTo(position + 15000 > totalDuration ? totalDuration : position + 15000);
                    })
                })
            }else{
                console.error("Classe .forward-button não especificada no elemento.")
            }
        }else{
            console.error("URL do podcast não especificada.")
        }
    })
}
